...
class myReporter(object):
    def __init__(self, current_user_id=1):
        ...
        self.current_user_id = current_user_id  # ✅ 保存用户ID
    ...
    def _fetch_data(self, patient_id):
        """从数据库获取所有需要的数据"""
        ...
        cursor.execute("""
            SELECT 
                诊所名称, 医生, 配药, 诊所地址, 诊所电话, 
                CAST(诊金 AS DECIMAL(10,2)) AS 诊金,
                处方抬头
            FROM 基本设置 
            WHERE user_id = %s
        ...
        if not base_data:
            ...
            # 确保每个字段都有默认值
            base_data = {
                k: (v if v is not None else "")  # 将None转为空字符串
        ...
        # +++ 新增处理：确保medical_data是字典 +++
        if medical_data is None:
            medical_data = {}
        else:
            # 可选：清理None值
            medical_data = {k: (v if v is not None else "") for k, v in medical_data.items()}
        ...
        default_medical_data = {
            ...
            '病证': '',
            # 药物相关字段
            **{f'药物{i}': '' for i in range(1, 21)},
            **{f'用量{i}': '' for i in range(1, 21)},
            **{f'先煎后下{i}': '' for i in range(1, 21)}
        }

        # 合并 medical_data（覆盖默认值）
        if medical_data:
            default_medical_data.update(medical_data)
    ...
    def _draw_page_header(self, c):
        ...
        clinic_name = self.data.get("诊所名称", "")  # 空字符串兜底
        prescription_title = self.data.get("处方抬头", "")
        ...
        # 手动插入空格加宽字间距
        spaced_clinic_name = " ".join(list(clinic_name))  # 在每个字符间插入一个空格
        c.drawCentredString(self.page_width / 2, self.y_position, spaced_clinic_name)
        self.y_position -= 0.7 * cm  # 诊所名称与主标题之间的行间距

        # 主标题
        c.setFont(self.font_name, 18)
        c.drawCentredString(self.page_width / 2, self.y_position, self.data["处方抬头"])
    ...
    def _draw_text_box(self, c, x, y, width, height, contents):
        """绘制带文本的边框区域"""
        c.rect(x, y, width, height)
        text_y = y + height - 0.8 * cm
        for text, font in contents:
            # 确保 text 不是 None，如果是则替换为默认空字符串
            safe_text = text or ""  # 如果 text 是 None，则赋值为 ""
            c.setFont(font, 13)
            c.drawString(x + 0.5 * cm, text_y, safe_text)
            text_y -= 1.2 * cm
    ...
    def _draw_footer(self, c):
        """绘制每页固定页脚"""
        footer_start_y = self.margin + self.footer_height
        current_y = self.margin + self.footer_height

        # 医生和配药信息处理
        has_doctor_or_dispensing = False  # 标记是否有医生或配药信息
        if self.data.get('医生') or self.data.get('配药'):
            # 绘制医生信息（从基本设置获取）
            if self.data.get('医生'):  # ✅ 数据来自基本设置
                lineDoctor = f"医生：{self.data['医生']}"
                c.drawString(self.page_width / 2 - 4 * cm, current_y, lineDoctor)
            # 绘制配药信息（从基本设置获取）
            if self.data.get('配药'):  # ✅ 数据来自基本设置
                linedrug = f"配药：{self.data['配药']}"
                c.drawRightString(self.page_width - 6 * cm, current_y, linedrug)
            # 无论是否有信息，都下移位置
            current_y -= 0.8 * cm
            has_doctor_or_dispensing = True

        fee_line = []
        # 检查并添加不为空的费用项
        if self.data.get('诊金') not in (None, "", 0):  # ✅ 原错误代码：self.data.get('诊金1')
            fee_line.append(f"诊金：{float(self.data['诊金']):.2f}元")  # ✅ 原错误代码：self.data['诊金1']
        if self.data.get('药费') is not None and str(self.data.get('药费')).strip():
            fee_line.append(f"药费：{self.data['药费']}元")
        if self.data.get('针灸费用') is not None and str(self.data.get('针灸费用')).strip():
            fee_line.append(f"针灸等费用：{self.data['针灸费用']}元")
        if self.data.get('总费用') is not None and str(self.data.get('总费用')).strip():
            fee_line.append(f"合计：{self.data['总费用']}元")

        if fee_line:
            fee_line_text = ' '.join(fee_line)
        ...
        contact_line = []
        # 检查并添加不为空的费用项
        if self.data.get('诊所地址') is not None and str(self.data.get('诊所地址')).strip():
            contact_line.append(f"地址：{self.data['诊所地址']}")  # ✅ 数据来自基本设置表
        if self.data.get('诊所电话') is not None and str(self.data.get('诊所电话')).strip():
            contact_line.append(f"电话：{self.data['诊所电话']}")  # ✅ 数据来自基本设置表
        if contact_line:
            contact_line_text = '                                '.join(contact_line)
            c.setFont(self.font_name, 14)
            c.drawString(2 * cm, current_y, contact_line_text)
            current_y -= 0.8 * cm
    ...
    def _draw_prescription_content(self, c):
        ...
        # 2. 打印证候（即病证表现，需要分页保护）
        # 当withoutdiagprint为False时显示病证（对应m_pbWithDiagSyndrome点击）
        # self.withoutdiagprint=True表示不显示病证（对应m_pbWithoutDiagSyndrome点击）
    ...
    def _draw_double_underline(self, c):
        """绘制双下划线"""
        for offset in [0.3, 0.6]:
            c.line(2 * cm, self.y_position - offset * cm,
                   self.page_width - 2 * cm, self.y_position - offset * cm)
    ...
    def generate(self, patient_id, withoutdiagprint):
        """✅ 修改点4：确保参数传递正确性"""
        # 参数说明：
        # withoutdiagprint=True 时不显示病证（对应m_pbWithoutDiagSyndrome按钮）
        # withoutdiagprint=False 时显示病证（对应m_pbWithDiagSyndrome按钮）
        self.withoutdiagprint = withoutdiagprint
        self.data = self._fetch_data(patient_id)
        buffer = BytesIO()
        ...
        # 绘制正文内容
        content_complete = self._draw_prescription_content(c)
    ...
    # 以下辅助方法保持不变（_draw_wrapped_text, add_newlines_by_width等）
    # 需要添加_calculate_text_height方法
